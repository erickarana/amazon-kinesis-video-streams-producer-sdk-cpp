...
{
    _CustomData() : first_video_frame(true),
                    eos_triggered(false),
...
void putEventMetadataMKVTags(CustomData *data, bool is_start_event, uint64_t event_timestamp, std::string &intervalId, long long cameraStartTime)
{
    ...
    std::string videoDeviceStr = std::string(data->stream_name);

    std::regex patron("(event|stream|live|manual)\\w*-(.+)");
    std::smatch resultado;
    string deviceId;
    if (std::regex_search(videoDeviceStr, resultado, patron))
    {
        if (resultado.size() > 2)
        { // Verifica que haya suficientes sub-coincidencias
            // Imprime la parte después del guion
            std::cout << "Parte extraída: " << resultado[2] << std::endl;
            deviceId = resultado[2];
        }
    }
    else
    {
    ...
    std::string event_interval_id_value = intervalIdtest;
